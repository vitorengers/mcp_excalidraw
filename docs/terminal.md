...
the sequences back into slot *names* and the block paints each run out of the same
`terminalXtermTheme` table the emulator in the tab beside it is themed from. A theme toggle
repaints both.
...
- `scripts/check-agent-stream-render-colour-browser.mjs` — and the half compiling cannot answer:
  ...
  Its stub strips the fold marks #246 added, and that is what keeps it an *emulator's* question:
