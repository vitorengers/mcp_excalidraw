...
`src/server.ts`. 74 routes, and the only surface that is workspace-aware — everything the
...
`scripts/check-caller-guard.mjs` holds the question,
`scripts/check-board-reads-guard.mjs` the reads and
