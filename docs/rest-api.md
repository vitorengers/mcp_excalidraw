...
| `GET /api/pair/status` | What became of a `requestId`: `pending`, or `approved` **once**, carrying the secret. Every poll after that answers `unknown`, which is also what a `requestId` nobody issued answers |
...
| `POST /api/pair/approve` | Approve one of them by `requestId` **and** `code`; mints the secret, writes the device to `devices.json`, and answers without the secret in it (loopback **caller** only) |
...
choosing between requests instead of confirming that one exists; the secret is handed over on
