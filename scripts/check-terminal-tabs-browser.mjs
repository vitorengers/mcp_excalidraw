...
try {
  ...
  // Which side, and not only "some other place". Since #200 the canvas reads
  // mirror | terminals | documentation and the region grows **rightward**, into the
  // documentation — which is why the reorder came with the push: `documentationClearance`
  // moves the board's own content out of the way by exactly the room the new block took.
  // "Not overlapping the source" passes for either side, which is how the direction went
  // unnoticed once already, so the side itself is what is asserted here.
  ...
  check('the detached block is entirely right of the one it came out of',
        Boolean(fromBlock) && Boolean(awayBlock) && awayBlock.x >= fromBlock.x + fromBlock.w,
        geometry);
  check('and keeps its size and its top edge',
        ...
        geometry);
  // The documentation is what the region grew into, and it gave way rather than being drawn
  // over. No `githubProject` is configured for this run, so the mirror is dormant and these
  // are the only two regions on the board.
  const authoredLeft = Math.min(...scene.authored.map((element) => element.x));
  check('and the documentation stepped aside for it, one gap clear of the region',
        Boolean(awayBlock) && Number.isFinite(authoredLeft)
        && Math.abs(authoredLeft - (awayBlock.x + awayBlock.w) - 120) < 1,
        geometry);
