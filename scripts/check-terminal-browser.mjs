...
try {
  ...
  check('the prompt takes focus', await evaluate("document.activeElement.className.includes('terminal-card__input')"));
  await typeText('pwd');
  check('what was typed is in the prompt',
        (await evaluate("document.querySelector('.terminal-card__input').value")) === 'pwd');
  ...
  await waitFor(async () => containsPath((await evaluate(PROBE)).card?.text, projectDir),
                'the shell to answer pwd');
  scene = await evaluate(PROBE);
  await shot('03-pwd');
  check('the shell answered, in the block', containsPath(scene.card.text, projectDir),
        String(scene.card.text).slice(-200));
  check('and the prompt was cleared for the next command',
        (await evaluate("document.querySelector('.terminal-card__input').value")) === '');
